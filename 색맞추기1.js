...
function rotate(a,b){
  if (dir[0] < 0) return;
  dir[a*4+b] = (dir[a*4+b] + 1) % 5;
  document.getElementById("tile0"+a+b).innerHTML = arrow[dir[a*4+b]];
  updateboard(1);
}
...
function resetgame() {
  for (var i = 0; i < 4; i++) {
    for (var j = 0; j < 4; j++) {
      dir[i*4+j] = 0;
      document.getElementById("tile0"+i+j).innerHTML = '&nbsp;';
    }
  }
  updateboard(1);
...
function newpuz() {
  ...
  for (var i = 0; i < 4; i++) {
    for (var j = 0; j < 4; j++) {
      document.getElementById("tile0"+i+j).style.background = color[yrb[i*4+j]];
      if (yrb[i*4+j] === 3){
        document.getElementById("tile0"+i+j).style.color = 'white';
      }
      else document.getElementById("tile0"+i+j).style.color = 'black';
    }
  }
  updateboard(2);
